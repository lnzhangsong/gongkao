...
**2026-09-15 登记两项新问题（用户提出，均未动工）**：① **申论解析重写**——用户判定「申论这块解析并不好用，需要重写」，即不是再打磨一轮而是方向要变；同日完成的 A1–C5 里数据层（字段合并 / 规格单源 / 规范外留痕）与呈现形态无关可沿用，呈现层（谱系 / 反向索引 / 干扰项 / 同源标注）随新方案重做或废弃，**方案定下来前不再往呈现层加东西**。② **大作文辅助功能**——范围经用户补全为「**整体辅助编写：立意 → 素材 → 规范 → 衔接**」四环节全流程（现有 `/assist` 只覆盖第一环节「审题立意 + 作答框架」）。四环节各自的现状与空白、以及与 A1「批改不做」/A2「升格不做」裁定的边界，已逐条列进清单；并与飞轮第 4 项「轻量写作工作台」合并为一条主线。
...
**2026-09-15 真题页版心固定（按用户反馈）**：用户要求「文字解析的版心别再变」。此前两处都随窗口伸缩：正文栏 `minmax(0,900px)`（且 ≥1600 加宽到 1160、≥2400 到 1420）、解析面板 body `max-width:1244px`。现引入令牌 `--measure: 1000px`（正文栏）与 `--measure-panel: 1244px`（解析面板），真题页挂 `.exam-layout`。

> - `.reading-layout.exam-layout`（两个类，胜过 reading.css 各断点里的单类规则）把正文栏钉在 `--measure`，并自带 `@media (min-width:801px)`——否则会顶掉 ≤800px 的单列布局。
> - 实测（真实浏览器）：窗口 1440 / 1600 / 1920 / 2400 → 正文栏恒 1000px（此前会涨到 1160/1420）；≤800px 仍是单列（右栏由汉堡菜单接管）。1300px 窗口下正文栏 908px、1020px 下 628px——正文旁边固定占着 280px 右栏（行文思路/干扰项/作答要求），窗口窄于 ~1440 时放不下 1000，只能按比例缩，否则横向溢出。
> - 解析面板版心 = `--measure-panel`，窗口 ≥1324px 时恒为 1244。
> - 只动真题页：阅读页的 1244 版心仍按原样（连同「1920 页壳收敛」留在优化清单 Phase 2 里）。
...
| 申论方法论书融入方案.md         | 《申论写作八讲》整书独立内容线 /method：EPUB 解析管线、单元数据模型、阅读页与方法卡注入 AI（M-2「通用方法论独立承载」的兑现）                                                                         |
| 数据飞轮与壁垒设计.md           | 壁垒演进顶层设计：飞轮四环节审计 + ROI 排序建议（使用信号回写为最高优先级）                                                                                                                           |
| 设计与体验优化清单.md           | **当前待办**（**🔴 申论解析重写（最高优先级）** / **🟠 大作文辅助功能（立意→素材→规范→衔接 全流程）** / M1 待实测 / Paper OS Phase 2 / 飞轮四项）+ OpenUI 评估结论（不引入）+ 历史批次归档            |
...
| M2 方法论书融入     | 《申论写作八讲》整书入库（EPUB → data/shenlun-book → shenlun_book 表 → /api/shenlun-book），/method 阅读页（目录/进度/锚点）+ 题型方法卡注入 /assist 审题 prompt；兑现 M-2「通用方法论独立承载」                              | 数据线（A 范式）+ AI 线     | ✅ 已落地（2026-09-15，见《申论方法论书融入方案.md》；读书记录进云同步/复习算法为后续）                                                          |
