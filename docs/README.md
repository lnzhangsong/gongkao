...
**2026-09-15 登记两项新问题（用户提出，均未动工）**：① **申论解析重写**——用户判定「申论这块解析并不好用，需要重写」，即不是再打磨一轮而是方向要变；同日完成的 A1–C5 里数据层（字段合并 / 规格单源 / 规范外留痕）与呈现形态无关可沿用，呈现层（谱系 / 反向索引 / 干扰项 / 同源标注）随新方案重做或废弃，**方案定下来前不再往呈现层加东西**。② **大作文辅助功能**——现有 `/assist` 只覆盖「审题立意 + 作答框架」，若用户要的是成文/升格/批改，则与 A1「批改不做」、A2「升格不做」的既有裁定冲突，需重新裁定。

> 两条都先要「说清要什么」再动工：解析重写要先列「不好用」清单（否则没有验收标准）；大作文辅助要先确认是审题/框架之外的哪一种。详见《设计与体验优化清单.md》§一 顶部两节。
...
| 设计与体验优化清单.md           | **当前待办**（**🔴 申论解析重写（最高优先级）** / **🟠 大作文辅助功能（范围待定）** / M1 待实测 / Paper OS Phase 2 / 飞轮四项）+ OpenUI 评估结论（不引入）+ 历史批次归档                              |
