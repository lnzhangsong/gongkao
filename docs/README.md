...
> - B2：`mode` 认不出六类、`sourceIdx` 不在本卷（越界或非数字）时不再静默兜底，改为在要点上留痕 `nonstandard: { field, got, used }[]`；要点卡顶部提示「加工方式 AI 给的是「X」，不在六类里，已按「归纳」计」。字段本身仍取合法值，掌握度/学习事件统计不被脏值污染；用户行内改过该字段即清除对应提示（`applyPointPatch`，只清改过的那条，不误伤）。
...
| 设计与体验优化清单.md           | **当前待办**（M1 待实测 / **M1 解析呈现优化 C1–C5**（A1 字段合并、B1 规格单源、B2 停止静默改值已完成）/ Paper OS Phase 2 / 飞轮四项）+ OpenUI 评估结论（不引入）+ 历史批次归档（2026-09-05 重组为两层） |
