...
export function LoadingScreen({ hint = '正在为你打开阅读空间…', finishing = false }: LoadingScreenProps) {
  ...
  // 进度：正常阶段递增模拟（封顶 88）；收尾阶段的 100% 由 finishing 派生，无需在 effect 里 setState
  useEffect(() => {
    if (finishing) return
    let cur = 0
  ...
  const displayPct = finishing ? 100 : pct

  return (
    <div className="loading-screen" role="status" aria-label="加载中">
      <div className="loading-stage">
        {/* 背景进度层：accent 色从左到右填充（跟随进度） */}
        <div className="loading-bg-progress" style={{ transform: `scaleX(${displayPct / 100})` }} />
        ...
        <div className="loading-progress">
          ...
          <span className="loading-pct">{displayPct}%</span>
        </div>
