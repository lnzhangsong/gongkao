 ...
 * 图片缺失 Pillow 时会回退落盘 .png（parse-xingce-images.py），故一并匹配。
 * 查询返回 DataUrls 组件可用的 JSON 数组字符串（含 w/h 供 <img> 预留布局），无图返回 null。
 */
const modules = import.meta.glob('../../data/xingce-img/**/*.{webp,png}', {
  eager: true,
  query: '?url',
  import: 'default',
...
for (const [file, url] of Object.entries(modules)) {
  const m = file.match(/xingce-img\/([^/]+)\/([qg]\d+)(?:_(\d+))?\.(?:webp|png)$/)
  if (!m) continue
